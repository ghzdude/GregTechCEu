...
public class MultiblockUIFactory<T extends MultiblockWithDisplayBase> {
    ...
    protected final GuiSyncManager guiSyncManager;
    ...
    public MultiblockUIFactory(@NotNull T mte, @NotNull PosGuiData guiData, @NotNull GuiSyncManager guiSyncManager) {
        this.mte = mte;
    ...
    protected void createExtraButton() {
        ...
        mte.createExtraButtons(rootPanel, guiSyncManager, list);
        if (list.isEmpty()) {
            list.add(new Widget<>()
        ...
        var panelSH = new SimplePanelSH(rootPanel, (m, sm) -> panel);
        guiSyncManager.syncValue("extra_buttons_panel", panelSH);
    ...
    protected void createVoidingButton() {
        if (mte.shouldShowVoidingModeButton()) {
            ...
            guiSyncManager.syncValue("voiding_state", voidingValue);
